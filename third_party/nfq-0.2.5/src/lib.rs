...
impl Message {
    ...
    pub fn get_hw_addr(&self) -> Option<&[u8]> {
        ...
        unsafe {
            ...
            // VENDORING NOTE: upstream 0.2.5 wrote `&(*self.hwaddr).hw_addr[..len]`,
            // which relies on an implicit autoref through a raw-pointer deref that
            // newer rustc denies by default (dangerous_implicit_autorefs). Made the
            // reference explicit; behavior is unchanged.
...
impl Queue {
    /// Returns the raw file descriptor of this queue's underlying netlink
    /// socket, for registering with an external event loop (e.g. epoll).
    ///
    /// VENDORING NOTE: this method does not exist in the published 0.2.5
    /// crate this file is otherwise an exact copy of -- see
    /// third_party/nfq-0.2.5/VENDORING.md for why this crate is vendored
