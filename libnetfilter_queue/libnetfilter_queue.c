                     ...
                     uint16_t queuenum, uint16_t pf)
{
    union
    {
        char buf[NFNL_HEADER_LEN + NFA_LENGTH(sizeof(struct nfqnl_msg_config_cmd))];
        struct nlmsghdr nmh;
    } u;
...
struct nfq_handle *nfq_open_nfnl(struct nfnl_handle *nfnlh)
{
    ...
    h->nfnlssh = nfnl_subsys_open(h->nfnlh, NFNL_SUBSYS_QUEUE, NFQNL_MSG_MAX, 0);
    if (!h->nfnlssh)
    {
        /* FIXME: nfq_errno */
        goto out_free;
...
int nfq_set_mode(struct nfq_q_handle *qh, uint8_t mode, uint32_t range)
{
    union
    {
        char buf[NFNL_HEADER_LEN + NFA_LENGTH(sizeof(struct nfqnl_msg_config_params))];
        struct nlmsghdr nmh;
    } u;
...
int nfq_set_queue_flags(struct nfq_q_handle *qh, uint32_t mask, uint32_t flags)
{
    union
    {
        char buf[NFNL_HEADER_LEN + NFA_LENGTH(sizeof(mask) + NFA_LENGTH(sizeof(flags)))];
        struct nlmsghdr nmh;
    } u;
                         ...
                         enum nfqnl_msg_types type)
{
    ...
    union
    {
        char buf[NFNL_HEADER_LEN + NFA_LENGTH(sizeof(mark)) + NFA_LENGTH(sizeof(vh))];
        struct nlmsghdr nmh;
    } u;
    ...
    nfnl_fill_hdr(qh->h->nfnlssh, &u.nmh, 0, AF_UNSPEC, qh->id, type, NLM_F_REQUEST);
...
{
    return __set_verdict(qh, id, verdict, 0, 0, data_len, buf, NFQNL_MSG_VERDICT);
}
...
{
    return __set_verdict(qh, id, verdict, htonl(mark), 1, data_len, buf, NFQNL_MSG_VERDICT);
}
...
int nfq_set_verdict_batch(struct nfq_q_handle *qh, uint32_t id,
                          uint32_t verdict)
{
    return __set_verdict(qh, id, verdict, 0, 0, 0, NULL, NFQNL_MSG_VERDICT_BATCH);
}
...
int nfq_set_verdict_mark(struct nfq_q_handle *qh, uint32_t id,
                         ...
                         uint32_t data_len, const unsigned char *buf)
{
    return __set_verdict(qh, id, verdict, mark, 1, data_len, buf, NFQNL_MSG_VERDICT);
}
...
{
    return nfnl_get_pointer_to_data(nfad->data, NFQA_PACKET_HDR, struct nfqnl_msg_packet_hdr);
}
