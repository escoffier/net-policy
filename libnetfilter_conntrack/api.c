 ...
 * \param type message type (see NFC_MSG_TYPE definition)
 ...
 */
int nfct_callback_register(struct nfct_handle *h, NFC_MSG_TYPE type,
                           int (*cb)(NFC_MSG_TYPE type, struct nf_conntrack *ct, void *data),
                           void *data)
{
    ...
    nfnl_callback_register(h->nfnlssh_ct, IPCTNL_MSG_CT_NEW, &h->nfnl_cb_ct);

    nfnl_callback_register(h->nfnlssh_ct, IPCTNL_MSG_CT_DELETE, &h->nfnl_cb_ct);
...
int nfct_callback_register2(struct nfct_handle *h,
                            NFC_MSG_TYPE type,
                            int (*cb)(const struct nlmsghdr *nlh, NFC_MSG_TYPE type, struct nf_conntrack *ct, void *data),
                            void *data)
{
    struct __data_container *container;
                         ...
                         const struct nfct_filter_dump *filter_dump)
{
    ...
    {
        nfnl_addattr32(&req->nlh, size, CTA_MARK, htonl(filter_dump->mark.val));
        nfnl_addattr32(&req->nlh, size, CTA_MARK_MASK, htonl(filter_dump->mark.mask));
    }
...
static void nfct_fill_hdr(struct nfnlhdr *req, uint16_t type, uint16_t flags, uint8_t l3num, uint8_t version)
{
...
static int __build_query_ct(struct nfnl_subsys_handle *ssh, const enum nf_conntrack_query qt, const void *data, void *buffer, unsigned int size)
{
    ...
    case NFCT_Q_GET:
        ...
        break;
    case NFCT_Q_FLUSH:
        nfct_fill_hdr(req, IPCTNL_MSG_CT_DELETE, NLM_F_ACK, *family, NFNETLINK_V0);
        break;
    case NFCT_Q_DUMP:
        nfct_fill_hdr(req, IPCTNL_MSG_CT_GET, NLM_F_DUMP, *family, NFNETLINK_V0);
        break;
    case NFCT_Q_DUMP_RESET:
        nfct_fill_hdr(req, IPCTNL_MSG_CT_GET_CTRZERO, NLM_F_DUMP, *family, NFNETLINK_V0);
        break;
    ...
    case NFCT_Q_DUMP_FILTER:
        nfct_fill_hdr(req, IPCTNL_MSG_CT_GET, NLM_F_DUMP, AF_UNSPEC, NFNETLINK_V0);
        __build_filter_dump(req, size, data);
        break;
    case NFCT_Q_DUMP_FILTER_RESET:
        nfct_fill_hdr(req, IPCTNL_MSG_CT_GET_CTRZERO, NLM_F_DUMP, AF_UNSPEC, NFNETLINK_V0);
        __build_filter_dump(req, size, data);
        break;
    default:
        errno = ENOTSUP;
 ...
 */
int nfct_parse_conntrack(NFC_MSG_TYPE type, const struct nlmsghdr *nlh, struct nf_conntrack *ct)
{
    unsigned int flags;
