...
nfct_build_tuple_ip(struct nlmsghdr *nlh, const struct __nfct_tuple *t)
{
    ...
    case AF_INET:
        ...
        break;
    case AF_INET6:
        mnl_attr_put(nlh, CTA_IP_V6_SRC, sizeof(struct in6_addr), &t->src.v6);
        mnl_attr_put(nlh, CTA_IP_V6_DST, sizeof(struct in6_addr), &t->dst.v6);
        break;
    default:
...
int nfct_build_tuple_raw(struct nlmsghdr *nlh, const struct __nfct_tuple *t)
{
...
nfct_build_nat_seq_adj(struct nlmsghdr *nlh, const struct nf_conntrack *ct,
                       int dir)
{
    int type = (dir == __DIR_ORIG) ? CTA_NAT_SEQ_ADJ_ORIG : CTA_NAT_SEQ_ADJ_REPLY;
    struct nlattr *nest;
