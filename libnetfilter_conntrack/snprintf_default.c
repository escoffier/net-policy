                                 ...
                                 const struct nf_conntrack *ct)
{
    return (snprintf(buf, len, "%-8s %u ",
                     l3proto2str[ct->head.orig.l3protonum] == NULL ? "unknown" : l3proto2str[ct->head.orig.l3protonum],
...
{
    return (snprintf(buf, len, "%-8s %u ",
                     proto2str[ct->head.orig.protonum] == NULL ? "unknown" : proto2str[ct->head.orig.protonum],
                                ...
                                const struct nf_conntrack *ct)
{
    return snprintf(buf, len, "%s ",
                    ct->protoinfo.tcp.state < TCP_CONNTRACK_MAX ? states[ct->protoinfo.tcp.state] : states[TCP_CONNTRACK_NONE]);
}
...
int __snprintf_connlabels(char *buf, unsigned int len,
                                 ...
                                 struct nfct_labelmap *map)
{
    ...
    ret = __snprintf_address(buf + offset, len, &ct->head.orig, "src", "dst");
    BUFFER_SIZE(ret, size, len, offset);
    ...
    ret = __snprintf_address(buf + offset, len, &ct->repl, "src", "dst");
    BUFFER_SIZE(ret, size, len, offset);
