...
#include "linux_nfnetlink_conntrack.h"
#include "linux_nf_conntrack_common.h"
 ...
 */
extern struct nfct_handle *nfct_open();
...
typedef enum {
    ...
    //custom
    NFCT_T_NEWUP = NFCT_T_NEW | NFCT_T_UPDATE,
...
/* comparison */
extern int nfct_compare(const struct nf_conntrack *ct1, const struct nf_conntrack *ct2);
...
extern int nfct_cmp(const struct nf_conntrack *ct1, const struct nf_conntrack *ct2, unsigned int flags);
...
extern int nfct_query(struct nfct_handle *h, const enum nf_conntrack_query query, const void *data);

extern int nfct_send(struct nfct_handle *h, const enum nf_conntrack_query query, const void *data);
...
int nfct_parse_conntrack(NFC_MSG_TYPE msg,
