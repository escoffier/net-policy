...
static void copy_attr_orig_ipv4_src(struct nf_conntrack *dest, const struct nf_conntrack *orig)
{
    dest->head.orig.src.v4 = orig->head.orig.src.v4;
}

static void copy_attr_orig_ipv4_dst(struct nf_conntrack *dest, const struct nf_conntrack *orig)
{
    dest->head.orig.dst.v4 = orig->head.orig.dst.v4;
}

static void copy_attr_repl_ipv4_src(struct nf_conntrack *dest, const struct nf_conntrack *orig)
{
    dest->repl.src.v4 = orig->repl.src.v4;
}

static void copy_attr_repl_ipv4_dst(struct nf_conntrack *dest, const struct nf_conntrack *orig)
{
    dest->repl.dst.v4 = orig->repl.dst.v4;
}

static void copy_attr_orig_ipv6_src(struct nf_conntrack *dest, const struct nf_conntrack *orig)
{
    memcpy(&dest->head.orig.src,  &orig->head.orig.src, sizeof(union __nfct_address));
}

static void copy_attr_orig_ipv6_dst(struct nf_conntrack *dest, const struct nf_conntrack *orig)
{
    memcpy(&dest->head.orig.dst, &orig->head.orig.dst, sizeof(union __nfct_address));
}

static void copy_attr_repl_ipv6_src(struct nf_conntrack *dest, const struct nf_conntrack *orig)
{
    memcpy(&dest->repl.src, &orig->repl.src, sizeof(union __nfct_address));
}

static void copy_attr_repl_ipv6_dst(struct nf_conntrack *dest, const struct nf_conntrack *orig)
{
    memcpy(&dest->repl.dst, &orig->repl.dst, sizeof(union __nfct_address));
}

static void copy_attr_orig_port_src(struct nf_conntrack *dest, const struct nf_conntrack *orig)
{
    dest->head.orig.l4src.all = orig->head.orig.l4src.all;
}

static void copy_attr_orig_port_dst(struct nf_conntrack *dest, const struct nf_conntrack *orig)
{
    dest->head.orig.l4dst.all = orig->head.orig.l4dst.all;
}

static void copy_attr_repl_port_src(struct nf_conntrack *dest, const struct nf_conntrack *orig)
{
    dest->repl.l4src.all = orig->repl.l4src.all;
}

static void copy_attr_repl_port_dst(struct nf_conntrack *dest, const struct nf_conntrack *orig)
{
    dest->repl.l4dst.all = orig->repl.l4dst.all;
}

static void copy_attr_orig_zone(struct nf_conntrack *dest, const struct nf_conntrack *orig)
{
    dest->head.orig.zone = orig->head.orig.zone;
}

static void copy_attr_repl_zone(struct nf_conntrack *dest, const struct nf_conntrack *orig)
{
    dest->repl.zone = orig->repl.zone;
}

static void copy_attr_icmp_type(struct nf_conntrack *dest, const struct nf_conntrack *orig)
{
    dest->head.orig.l4dst.icmp.type = orig->head.orig.l4dst.icmp.type;
}

static void copy_attr_icmp_code(struct nf_conntrack *dest, const struct nf_conntrack *orig)
{
    dest->head.orig.l4dst.icmp.code = orig->head.orig.l4dst.icmp.code;
}

static void copy_attr_icmp_id(struct nf_conntrack *dest, const struct nf_conntrack *orig)
{
    dest->head.orig.l4src.icmp.id = orig->head.orig.l4src.icmp.id;
}

static void copy_attr_orig_l3proto(struct nf_conntrack *dest, const struct nf_conntrack *orig)
{
    dest->head.orig.l3protonum = orig->head.orig.l3protonum;
}

static void copy_attr_repl_l3proto(struct nf_conntrack *dest, const struct nf_conntrack *orig)
{
    dest->repl.l3protonum = orig->repl.l3protonum;
}

static void copy_attr_orig_l4proto(struct nf_conntrack *dest, const struct nf_conntrack *orig)
{
    dest->head.orig.protonum = orig->head.orig.protonum;
}

static void copy_attr_repl_l4proto(struct nf_conntrack *dest, const struct nf_conntrack *orig)
{
    dest->repl.protonum = orig->repl.protonum;
}

static void copy_attr_master_ipv4_src(struct nf_conntrack *dest, const struct nf_conntrack *orig)
{
    dest->master.src.v4 = orig->master.src.v4;
}

static void copy_attr_master_ipv4_dst(struct nf_conntrack *dest, const struct nf_conntrack *orig)
{
    dest->master.dst.v4 = orig->master.dst.v4;
}

static void copy_attr_master_ipv6_src(struct nf_conntrack *dest, const struct nf_conntrack *orig)
{
    memcpy(&dest->master.src, &orig->master.src,
           sizeof(union __nfct_address));
}

static void copy_attr_master_ipv6_dst(struct nf_conntrack *dest, const struct nf_conntrack *orig)
{
    memcpy(&dest->master.dst, &orig->master.dst,
           sizeof(union __nfct_address));
}

static void copy_attr_master_port_src(struct nf_conntrack *dest, const struct nf_conntrack *orig)
{
    dest->master.l4src.all = orig->master.l4src.all;
}

static void copy_attr_master_port_dst(struct nf_conntrack *dest, const struct nf_conntrack *orig)
{
    dest->master.l4dst.all = orig->master.l4dst.all;
}

static void copy_attr_master_l3proto(struct nf_conntrack *dest, const struct nf_conntrack *orig)
{
    dest->master.l3protonum = orig->master.l3protonum;
}

static void copy_attr_master_l4proto(struct nf_conntrack *dest, const struct nf_conntrack *orig)
{
    dest->master.protonum = orig->master.protonum;
}

static void copy_attr_tcp_state(struct nf_conntrack *dest, const struct nf_conntrack *orig)
{
...
static void *do_copy_attr_connlabels(struct nfct_bitmask *dest, const struct nfct_bitmask *orig)
{
    if (orig == NULL) return dest;
    if (dest) nfct_bitmask_destroy(dest);
    return nfct_bitmask_clone(orig);
}

static void copy_attr_connlabels(struct nf_conntrack *dest, const struct nf_conntrack *orig)
{
