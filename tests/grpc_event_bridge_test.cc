...
/*Shared across every TEST() in this file, mirroring how the removed
 *g_event_bridge singleton used to be implicitly shared.*/
grpc_bridge::EventBridge bridge;
...
TEST(EventBridge, PublishPolicyMatchThenWaitAndPopReturnsEvent) {
  ...
  bridge.PublishPolicyMatch(tuple, NetPolicyRule::kAllow, FlowDir::kIngress,
                                                     "test-policy");

  netpolicy::v1::PolicyEvent event;
  ASSERT_TRUE(bridge.WaitAndPop(&event, std::chrono::milliseconds(100)));
...
TEST(EventBridge, WaitAndPopTimesOutWhenQueueEmpty) {
  ...
  while (bridge.WaitAndPop(&drain, std::chrono::milliseconds(1))) {
  }
  netpolicy::v1::PolicyEvent event;
  EXPECT_FALSE(bridge.WaitAndPop(&event, std::chrono::milliseconds(50)));
}
...
TEST(EventBridge, QueueDropsOldestWhenFullAndLogsWarning) {
  ...
  popped++;
  while (bridge.WaitAndPop(&event, std::chrono::milliseconds(1)))
    popped++;
