                        ...
                        struct nlmsghdr *hdr)
{
    ...
    if (!h)
    {
        rtnl_log(LOG_D, "no registered handler for type %u", type);
        return 0;
    }
...
{
    rtnl_log(LOG_D, "registering handler for type %u",
             hdlr->nlmsg_type);
...
int rtnl_handler_unregister(struct rtnl_handle *rtnl_handle, struct rtnl_handler *hdlr)
{
    ...
    rtnl_log(LOG_D, "unregistering handler for type %u", hdlr->nlmsg_type);
...
int rtnl_dump_type(struct rtnl_handle *rtnl_handle, unsigned int type)
{
    ...
    return sendto(rtnl_handle->rtnl_fd, &req, sizeof(req), 0, (struct sockaddr *)&nladdr, sizeof(nladdr));
}
...
int rtnl_receive(struct rtnl_handle *rtnl_handle)
{
    ...
    if (status == 0)
    {
        rtnl_log(LOG_E, "EOF on rtnl socket");
        return -1;
    }
    if (msg.msg_namelen != sizeof(nladdr))
    {
        rtnl_log(LOG_E, "invalid address size");
        return -1;
    }
    ...
    while (NLMSG_OK(h, status))
    {
        ...
        if (h->nlmsg_type == NLMSG_ERROR)
        {
            ...
            if (h->nlmsg_len >= NLMSG_LENGTH(sizeof(struct nlmsgerr)))
                errno = -err->error;
            rtnl_log(LOG_E, "NLMSG_ERROR, errnp=%d",
                     errno);
            return -1;
        }
...
struct rtnl_handle *rtnl_open(void)
{
    ...
    if (h->rtnl_fd < 0)
    {
        rtnl_log(LOG_E, "unable to create rtnetlink socket");
        goto err;
    }
    ...
    {
        rtnl_log(LOG_E, "unable to bind rtnetlink socket");
        goto err_close;
    }

    if (getsockname(h->rtnl_fd, (struct sockaddr *)&h->rtnl_local, &addrlen) < 0)
    {
        rtnl_log(LOG_E, "cannot gescockname(rtnl_socket)");
        goto err_close;
    }

    if (addrlen != sizeof(h->rtnl_local))
    {
        rtnl_log(LOG_E, "invalid address size %u", addr_len);
        goto err_close;
    }

    if (h->rtnl_local.nl_family != AF_NETLINK)
    {
        rtnl_log(LOG_E, "invalid AF %u", h->rtnl_local.nl_family);
        goto err_close;
    }
