...
void nfnl_dump_packet(struct nlmsghdr *nlh, int received_len, char *desc)
{
    ...
    printf("  NLMSG_DATA(nlh) = %p (+%td bytes)\n", nlmsg_data, (nlmsg_data - (void *)nlh));
    printf("  NFM_NFA(NLMSG_DATA(nlh)) = %p (+%td bytes)\n", nfa, ((void *)nfa - (void *)nlh));
    printf("  NFM_PAYLOAD(nlh) = %u\n", len);
    printf("  nlmsg_type = %u, nlmsg_len = %u, nlmsg_seq = %u nlmsg_flags = 0x%x\n", 
            nlh->nlmsg_type, nlh->nlmsg_len, nlh->nlmsg_seq, nlh->nlmsg_flags);
...
static int recalc_rebind_subscriptions(struct nfnl_handle *nfnlh)
{
    ...
    err = bind(nfnlh->fd, (struct sockaddr *)&nfnlh->local, sizeof(nfnlh->local));
    if (err == -1)
        return -1;
 ...
 */
struct nfnl_subsys_handle *nfnl_subsys_open(struct nfnl_handle *nfnlh, uint8_t subsys_id, uint8_t cb_count, uint32_t subscriptions)
{
...
{
    assert(nfnlh);
    return setsockopt(nfnlh->fd, SOL_NETLINK, NETLINK_ADD_MEMBERSHIP, &group, sizeof(group));
}
...
int nfnl_send(struct nfnl_handle *nfnlh, struct nlmsghdr *n)
{
    ...
    return sendto(nfnlh->fd, n, n->nlmsg_len, 0, (struct sockaddr *)&nfnlh->peer, sizeof(nfnlh->peer));
}
...
nfnl_recv(const struct nfnl_handle *h, unsigned char *buf, size_t len)
{
    ...
    status = recvfrom(h->fd, buf, len, 0, (struct sockaddr *)&peer, &addrlen);
    if (status <= 0)
        return status;
 ...
 *
 */
int nfnl_nfa_addattr16(struct nfattr *nfa, int maxlen, int type, uint16_t data)
{
    assert(nfa);
    assert(maxlen > 0);
    assert(type >= 0);
 ...
 *
 */
int nfnl_addattr16(struct nlmsghdr *n, int maxlen, int type, uint16_t data)
{
    assert(n);
    assert(maxlen > 0);
    assert(type >= 0);
 ...
 *
 */
int nfnl_nfa_addattr32(struct nfattr *nfa, int maxlen, int type, uint32_t data)
{
    assert(nfa);
    assert(maxlen > 0);
    assert(type >= 0);
 ...
 *
 */
int nfnl_addattr32(struct nlmsghdr *n, int maxlen, int type, uint32_t data)
{
    assert(n);
    assert(maxlen > 0);
    assert(type >= 0);
 ...
 * appropiately.
 */
int nfnl_callback_register(struct nfnl_subsys_handle *ssh, uint8_t type, struct nfnl_callback *cb)
{
    assert(ssh);
    assert(cb);
